...
# from module.emb_position import *
...
class ACMIL_MHA(nn.Module):
    def __init__(self, conf, n_token=1, n_masked_patch=0, mask_drop=0):
        ...
        self.dimreduction = DimReduction(conf['input_size'], conf['inner_size'])
        self.sub_attention = nn.ModuleList()
        for i in range(n_token):
            self.sub_attention.append(MutiHeadAttention(conf['inner_size'], 8, n_masked_patch=n_masked_patch, mask_drop=mask_drop))
        self.bag_attention = MutiHeadAttention_modify(conf['inner_size'], 8)
        self.q = nn.Parameter(torch.zeros((1, n_token, conf['inner_size'])))
        nn.init.normal_(self.q, std=1e-6)
        self.n_class = 2

        self.factor = conf['factor']
        self.multi_weights = AdapetiveScaleFusion(conf['input_size'], conf['fusion_type'], conf['alpha'], conf['concat_type'])
        self.ReduceDim = nn.Sequential(nn.Linear(conf['input_size']*self.factor, conf['input_size']), nn.ReLU())

        self.classifier = nn.ModuleList()
        for i in range(n_token):
            self.classifier.append(Classifier_1fc(conf['inner_size'], self.n_class, 0.0))
        self.n_token = n_token
        self.Slide_classifier = Classifier_1fc(conf['inner_size'], self.n_class, 0.0)
...
class ACMIL_GA(nn.Module):
    def __init__(self, conf, D=128, droprate=0, n_masked_patch=0, mask_drop=0):
        super(ACMIL_GA, self).__init__()
        self.n_tokens = conf['n_tokens']
        self.dimreduction = DimReduction(conf['input_size'], conf['inner_size'])
        self.attention = Attention_Gated(conf['inner_size'], D, self.n_tokens)
        self.classifier = nn.ModuleList()
        self.n_class = 2
        for i in range(self.n_tokens):
            self.classifier.append(Classifier_1fc(conf['inner_size'], self.n_class, droprate))
        self.n_masked_patch = n_masked_patch
        self.Slide_classifier = Classifier_1fc(conf['inner_size'], self.n_class, droprate)
        self.mask_drop = mask_drop
    ...
    def forward(self, x, multi_scale = True): ## x: N x L
        x = x.squeeze(0)
        # print(x.shape)
        if multi_scale == True:
...
if __name__ == '__main__':
    ...
    input_data = torch.randn(1, 100, 1536)
    model = ACMIL_GA(conf=config)
    outputs,slide_output,attn = model(input_data)
    print(outputs.shape,slide_output.shape,attn.shape)